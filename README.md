...
Both interviewees, independently, described the same behavior: they trust a high score by default when skimming quickly, but want to see the reasoning behind it the moment a decision actually matters (e.g. deciding whether to contact a researcher). Both also explicitly rejected the idea of a silently "corrected" score — both wanted the original score, the corrected score, and the underlying evidence shown together, never one replacing another. That is why this prototype never hides or overwrites the original score — it is always shown alongside the correction and the exact phrase that triggered it. See `docs/customer_interview_report.md` and `docs/interview_question_rationale.md` for the full interviews.
...
- Interview question drafting, product-direction ideation, and drafting of all submission documents (product brief, data validation report, interview reports) were AI-assisted; see `docs/ai_usage_note.md` for the full breakdown of what was AI-drafted versus team-verified, including a real bug the test suite caught in AI-assisted code.
